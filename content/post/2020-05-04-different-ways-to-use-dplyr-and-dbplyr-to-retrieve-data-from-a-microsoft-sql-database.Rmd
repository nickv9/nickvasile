...
dplyr::tbl(con, "People") %>%
  dplyr::select(email, first_name, last_name, account_id ) %>% 
...
dplyr::tbl(con, "Companies") %>% 
  ...
  summarise(companies = n()) %>% #n() is similiar to SQL count
...
tbl(con, dplyr::sql(query)) %>% 
  head(10) %>% #limit for speed during demo
  collect()

# Example 2: Use dplyr syntax to manipulate raw SQL query
```
