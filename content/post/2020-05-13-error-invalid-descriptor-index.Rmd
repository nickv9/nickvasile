...
First, I'll create a test database following [simple instructions from Microsoft](https://docs.microsoft.com/en-us/sql/t-sql/statements/create-database-transact-sql?view=sql-server-ver15#a-creating-a-database-without-specifying-files).
...
DBI::dbExecute(con,
               "IF DB_ID (N'dbi_error') IS NOT NULL DROP DATABASE dbi_error")
...
Next, I'll write two different sets of data:
...
1. Judging by the sheer frequency, and recency, of this issue being reported, I don't believe this is a corner case affecting a small minority of R users.
...
SOURCE: [Microsoft documentation on "Gettling Long Data"](https://docs.microsoft.com/en-us/sql/odbc/reference/develop-app/getting-long-data?view=sql-server-ver15)

2. The workaround is to put "long" columns at the end of the select statement, and there are a few different methods to do this.
...
* https://github.com/r-dbi/odbc/pull/359  <- looks like it adds better error messaging (tremendously valuable!) vs. fully addressing the problem
...
tbl_long_cols <- function(con, table, size_thresh = 200) {
    ...
    dplyr::tbl(con, table) %>%
        dplyr::select(-tidyselect::all_of(long_cols), tidyselect::everything(),
...
TODO: https://github.com/r-dbi/odbc/issues/309#issuecomment-615255613 
